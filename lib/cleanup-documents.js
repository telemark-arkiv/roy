...
module.exports = miss.through(function (chunck, encoding, callback) {
  ...
  if (item.errors.length === 0 && documents.length > 0) {
    ...
    if (item.restrictedAddressNotification && item.restrictedAddressNotification.document) {
    ...
    if (item.noGuardianFoundNotification && item.noGuardianFoundNotification.document) {
